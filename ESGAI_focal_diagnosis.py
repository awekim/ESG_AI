...
def AI_detect(transcript, business_description, trade_description):
    prompt = f"""
    Business Description: {business_description}
    Firm Overview: {transcript}
    Trade Description: {trade_description}
**Task**  
Given the business description, overview, and trade description of a company, classify it as an "AI company", "AI-related company", or "Others".  
...
esg_df = esg_df[['Acquiror_name', 'Acquiror_business_description','Acquiror_overview','Acquiror_trade_description']].drop_duplicates()
esg_df = esg_df[esg_df['Acquiror_overview'].notnull()].reset_index(drop=True)

acquiror_names = esg_df['Acquiror_name'].tolist()
firm_overview = esg_df['Acquiror_overview'].tolist()
firm_business = esg_df['Acquiror_business_description'].tolist()
firm_trade = esg_df['Acquiror_trade_description'].tolist()

results = []
for i, (name, overview, business, trade) in enumerate(zip(acquiror_names, firm_overview, firm_business, firm_trade)):
    print(f"[{i+1}/{len(acquiror_names)}] 처리 중...")
    try:
        status, explanation, confidence = AI_detect(overview, business, trade)
    except Exception as e:
        status, explanation, confidence = "Error", str(e)

    results.append({
        "Acquiror_name": name,
        "Acquiror_overview": overview,
        "Acquiror_business_description": business,
        "Acquiror_trade_description": trade,
